...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (migrator, from, to) async {
      ...
      if (from < 8) {
        try {
          await migrator.addColumn(statsCache, statsCache.practiceDaysAllTime);
        } catch (_) {}
      }
      if (from < 9) {
        try {
          await migrator.addColumn(preferences, preferences.guestUsername);
        } catch (_) {}
      }
      if (from < 10) {
        try {
          await migrator.addColumn(
            preferences,
            preferences.notificationPromptSnoozedUntilUtc,
          );
        } catch (_) {}
        try {
          await migrator.addColumn(
            preferences,
            preferences.notificationPromptSnoozedUntilSessions,
          );
        } catch (_) {}
      }
      if (from < 11) {
        try {
          await migrator.addColumn(
            statsCache,
            statsCache.weeklyMinutesByDayJson,
          );
        } catch (_) {}
      }
    },
