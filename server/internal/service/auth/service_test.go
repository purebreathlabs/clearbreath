...
func TestVerifyProviderDev(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sub, err := s.verifyProvider(context.Background(), tt.provider, tt.idToken, tt.deviceID, tt.header)
			if tt.wantCode == "" {
				if err != nil {
...
func TestVerifyProviderDevDisabled(t *testing.T) {
	...
	_, err := s.verifyProvider(context.Background(), "dev", "user1", "device1", "secret")
	if err == nil {
		t.Fatalf("expected error")
	}
	e, ok := apierr.As(err)
...
func TestVerifyProviderNotConfigured(t *testing.T) {
	...
	_, err := s.verifyProvider(context.Background(), "google", "token", "device1", "")
	if err == nil {
		t.Fatalf("expected error")
	}
	e, ok := apierr.As(err)
	...
	_, err = s.verifyProvider(context.Background(), "apple", "token", "device1", "")
	if err == nil {
		t.Fatalf("expected error")
	}
	e, ok = apierr.As(err)
...
func TestVerifyProviderUnsupportedProvider(t *testing.T) {
	s := &Service{}
	_, err := s.verifyProvider(context.Background(), "nope", "token", "device1", "")
	if err == nil {
		t.Fatalf("expected error")
	}
	e, ok := apierr.As(err)
...
func TestIsAutoGeneratedName(t *testing.T) {
	tests := []struct {
		...
		{name: "valid auto name", in: "Breather123456", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isAutoGeneratedName(tt.in)
			if got != tt.want {
				t.Fatalf("isAutoGeneratedName(%q): got %v, want %v", tt.in, got, tt.want)
			}
		})
	}
}
